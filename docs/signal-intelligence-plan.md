...
**Status:** PLANNED — SI-P1–SI-P5 · **Depends on:** the insight service (trait store, social listening/care, audiences), the intelligence LLM seam (`LlmRouter` FAST/SMART tiers, openai-compatible provider), the STT seam (`SpeechController` + mock-whisper), the bridge + connector doctrine (manual ch. 41), the AI audit ledger
