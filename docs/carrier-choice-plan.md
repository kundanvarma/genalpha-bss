...
**Status:** COMPLETE — C-P1–C-P4 shipped · suite #90 `carrier_choice_test` · **Depends on:** the logistics seam (`LogisticsClient`), the shipping-order flow, the checkout `simType` pattern, the Integrations console tab

> **C-P4 shipped (routing + reach):** rule-based **postcode routing** — a carrier serves a `postcode_prefix` (V7); a home booking with no explicit carrier routes by the longest matching prefix, else the default, else global (precedence: shopper's carrier → postcode rule → default → global). **PostNord** added as a third carrier (`PostnordCarrierAdapter`, reusing the Bring wire; `mock-postnord`). Console Logistics card gains a postcode-prefix field + the PostNord option. *Real credentials are already just config:* point a carrier's `baseUrl` at the real API and its `secretRef` at an env var holding the key — no code change (the mocks are only for offline proof, same honesty as the real-Strapi opt-in). Proven by suite #90 (extended): 0150→Bring, 9020→PostNord by rule; Helthjem global fallback + `fulfilment_test` stay green.
