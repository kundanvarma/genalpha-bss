...
| Tell the phone what it may use (device entitlements: VoLTE / VoWiFi / SMSoIP / data plan / eSIM ODSA) | GSMA TS.43 Entitlement Configuration Server (`/ts43`, EAP-AKA over HTTP against the AUC seam) + house `/tmf-api/deviceEntitlement/v1` | `device-entitlement` (+ `mock-hss`, `mock-ts43-device` in dev) | ✅ #124 `device_entitlement_test` — the plan's spec decides, the SIM proves itself (RFC 4187 EAP-AKA, forged answers refused), token reuse, the VoWiFi service flow, companion eSIM activation with an SGP.22 code, primary eSIM transfer, suspended lines answer DISABLED, tenant isolation; ◐ OEM certification (Apple/Google) is the operator's calendar; real AUC behind `AUC_BASE_URL`; see `docs/entitlement-seam.md` |
