...
| Describe the business to people, applications and agents, and gate agent writes through governed actions | Operational ontology (SID/ODA/TMF lineage) + MCP + generated SDK | `ontology` | ✅ #125 `ontology_test` — registry conformance against gateway routes, event sources, realm roles and the component's self-description; upgradeSubscription end to end with a receipt; ◐ one action so far, other components not yet self-describing at runtime |
