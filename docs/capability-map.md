...
| Say one true thing about a bill's situation, in every channel | `dueDate` on TMF678 + house `billSituation` block beside the standard payload | `billing` (`BillSituations`, pure) | ✅ `bill_situation_test` (eight situations with a fixed precedence computed once; an approved payment extension flips the back office, the CSR desk and the customer's app to `arrangement` together and stops the dunning chase; no channel owns a clock — the claims gate refuses one that grows a date comparison; ◐ no dated history, so "was it overdue on the 14th" cannot be asked) |
