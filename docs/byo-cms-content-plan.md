...
**Status:** COMPLETE — P1–P5 shipped (2026-08-11) · suite #89 `byo_cms_test.js` · **Depends on:** `services/document` (TMF667), existing `ContentStore` seam
...
>
> **P4:** the generic **`HttpCmsAssetProvider`** (`provider: http`) — config-driven upload (`multipart`|`raw`, file field, auth header), JSON-pointer asset extraction, `resolveBase` + query-style renditions — so any HTTP CMS is a config row, not a class. Config-serialisation fixed (nested JSON object stored as JSON). **Proven against both `mock-strapi` AND real self-hosted Strapi** whose wire is unlike Sanity's (multipart, array response, relative `/uploads` urls): bind→multipart upload→302 to the relative url→bytes served, zero vendor code. The real proof is an **opt-in `--profile strapi`** (`integrations/strapi`: scaffolds Strapi 5 + sqlite at build, a bootstrap grants the public role Upload permission so the connector uploads anonymously); the *same* `HttpCmsAssetProvider`, unchanged, drove genuine Strapi end-to-end (served real bytes from `/uploads/…`).
>
> **P5:** committed E2E **suite #89 `byo_cms_test.js`** (repeatable, self-cleaning) — hosted default serves bytes; Sanity reference 302s + rendition + CDN bytes; webhook delete→placeholder / upsert→cache-bust / forged→401; generic connector on `mock-strapi`; a **health-gated REAL-Strapi leg** (runs when `--profile strapi` is up, skips clean otherwise); per-tenant binding; unbind→hosted. `architecture.md` §content-seam + `capability-map` updated. Document module tests green on real Postgres (Testcontainers).
