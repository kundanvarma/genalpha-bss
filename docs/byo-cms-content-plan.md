...
>
> **P5:** committed E2E **suite #89 `byo_cms_test.js`** (9 checks, repeatable, self-cleaning) — hosted default serves bytes; Sanity reference 302s + rendition + CDN bytes; webhook delete→placeholder / upsert→cache-bust / forged→401; generic connector on Strapi's wire; per-tenant binding; unbind→hosted. `architecture.md` §content-seam + `capability-map` updated. Document module tests green on real Postgres (Testcontainers).
