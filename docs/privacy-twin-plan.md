...
**Status:** T-P1 SHIPPED 2026-08-19 (the deterministic twinning engine: `TwinningService` supersedes RedactionService — ONE pass yields the redacted stored text AND the twin, HMAC(per-signal key) surrogates from same-shaped Norwegian pools, offset map persisted; `PiiRecognizer` gains span-finding, `NameRecognizer` = the dictionary+capitalization floor (demo personas + ~90 NO/common names — recall is the honest limit), `NerRecognizer` = the config-enabled local-model seam (`bss.insight.ner-url`, fail-open); `twin_vault` V34+V35 RLS with party-keyed erasure — the orchestrator's erase now reports `twinKeysDestroyed`; `GET /signal/{id}/twin` shows the operator exactly what would leave. Proven live: "Kai Kunde… +47 99 88 77 66… kai@…" became "Eivind Lund… +47 43 18 42 61… vegard.haugen@example.net" — no real fact in the twin, in-signal consistency (same person = same fiction twice), cross-signal difference (signal 2 = "Sindre Tangen" — no longitudinal profile), the churn phrase verbatim-intact, 2 twin keys destroyed on erasure. signal_intelligence + martech + privacy suites green) · T-P2 SHIPPED 2026-08-19 (the battery reads the TWIN whenever one exists — proven in the ledger itself: the recorded frontier prompt says "Magnhild Sande ringte…", `real_name_leaked=false`; evidence returns in twin-space with `evidenceSpace:"twin"` and the STORE re-anchors each quote through the vault's offset map into stored-text space before the verbatim gate — shared segments copy, wholly-covered surrogates become their [TYPE] token, a quote that slices a surrogate is unmappable and drops (422); forged twin-space evidence refused; stored evidence carries no surrogate inward. Exposure class for signal classification is now FICTION, not redacted-real. signal_intelligence suite green end-to-end on the twin path) · T-P3 SHIPPED 2026-08-19 (exposure receipts on the ledger — `ai_audit` V24 gains `exposure` + `jurisdiction`, stamped by the governor per call from the use-case declaration registry (signal-classification→twin, voc-ask→aggregate, undeclared→raw — the honest default) with provider-derived jurisdiction; the RAW GATE: a raw use-case runs only for tenants with `ai-raw-exposure: true` in the fleet file (genalpha+nova opted in; new tenants default OFF — refused-raw-exposure lands on the ledger); console Privacy & governance → **AI data flows** pane: class chips + per-flow rows, live from the ledger, historical rows honestly `unlabelled`. Proven live: twin/aggregate/raw all labelled (the raw:1 is the copy assistant through the opted-in gate), 1432 pre-receipt rows unlabelled, martech suite green through the gate) · T-P4 SHIPPED 2026-08-19 (provable non-retention + the flywheel corpus — `ai_audit` V25 gains `canary`: every exposing call (exposure ≠ none) carries a unique `TVX-<12hex>` marker appended to the prompt as an ignore-this-line internal ref; `CanaryProbe` (daily schedule + `POST /ai/v1/canaryProbe`) later asks the provider to COMPLETE a stored canary's prefix with no context — a provider that trained on our traffic can, one that kept its word says UNKNOWN; each probe outcome lands on the ledger as an action (`no-retention-observed` accumulating is DPA-meeting evidence, one `RETENTION-SUSPECTED` is an incident). `GET /insight/v1/signal/flywheel/dataset` assembles the distillation corpus entirely in twin space — input = twin text, output = the frontier classification with evidence re-anchored BACK into twin space via the offset map (unmappable pair → dropped, never approximated) — a training set containing zero real facts, exportable without a DPIA. Proven live: canaries in the recorded prompts (`canary_in_prompt=t`, `real_name=f`), probe 2/2 no-retention-observed, 19-pair corpus with 41 evidence quotes verbatim in twin input and no real name/number anywhere, customer token refused (403) on corpus + twin endpoints; signal_intelligence + martech suites green. The LoRA training run itself is documented-not-built — the corpus assembler is the shipped seam) · **Depends on:** the signal store + PII firewall (`PiiRecognizer`
