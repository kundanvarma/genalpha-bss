...
> **P2 (per-tenant PSP + logistics config) shipped:** landed across the carrier arc (C-P2: `carrier_config` + Logistics card) and the PSP arc (PSP-P1: `payment_provider_config` + Payment card), then completed here — the Payment card exposes the **routing fields** (`priority`, `currencies` — the suite-#96 orchestration levers) and both cards grew **Test connection**: `POST /paymentProvider/{p}/test` and `POST /carrier/{c}/test`, a reachability probe of the provider's `/health` (3s connect timeout) that is **never a payment or a booking** — a bad base URL reads "✗ unreachable" on the card before checkout ever finds out. Global env stays the fallback throughout. Remaining after P2: P3 + P4 — see below.

> **P3 shipped (2026-08-12): the subscription-metrics engine.** `GET /revenue/v1/subscriptionMetrics` (+`?format=csv`) — an MRR waterfall computed from the subledger's own account-4000 rows per customer per month (billed truth, not catalog list price): new/expansion/contraction/churned classified against the prior month so the identity `mrr(m)=mrr(m-1)+new+expansion−contraction−churned` holds by construction; ARPU, churn rate, NRR; a drill-down of who moved the number; the Reporting panel grows the KPI tiles, waterfall table, drill-down and the console's first CSV download. Suite #100 checks the identity ARITHMETICALLY and cross-checks MRR against the independent `/summary` path.

> **P4 half-shipped (2026-08-12): the generic carrier connector.** `HttpCarrierAdapter` (carrier `http`) — any book-and-track wire rides on config alone: `bookPath` + JSON pointers (`trackingPointer` etc.) + `pickupPath` template, canonical booking body, auth header/prefix configurable, secret still an env-var ref. Proven by suite #101: pointed at the BRING-shaped mock — a wire it has zero code for — a real order books end-to-end (BRG… consignment). Carriers are safe to generic-connect because failure degrades to the manual warehouse flow; **money stays named-adapters, deliberately.**

> **P4's other half — the encrypted secret store: ASSESSED, NOT BUILT (deliberate).** "Paste-and-go" credentials would move secrets from the deployment layer (compose/K8s secrets → env vars, named by `secret_ref`) into the application database. That trade is wrong for this stack today: (1) the DB is multi-tenant and RLS-guarded but its backups/dumps are not envelope-encrypted — a leaked dump must never contain provider keys; (2) a proper build needs a KMS/master-key story (Vault, cloud KMS, or at minimum per-deployment envelope keys + rotation), which is deployment infrastructure, not a console feature; (3) every seam already works with refs, and operators deploying with Helm get K8s Secrets natively. The honest v1 remains: the console shows WHICH env var a seam needs and the Test button proves it resolves. Revisit only alongside a real KMS integration.
