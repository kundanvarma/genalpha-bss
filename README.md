...
| ontology | Operational Semantic Registry `/ontology/v1` + MCP | 8160 | **The BSS knows what it can do** — the GenAlpha Operational Ontology as a component: business concepts with SID/TMF lineage, **governed actions** (meaning, typed preconditions, permissions, policy domain, governance, the capability that executes, typed effects, emitted events, what settles them), capabilities and components — a YAML registry in the repo, schema-validated and reference-checked at startup, served merged with an operator's own overlay (Taranga adds a price ceiling; core conditions can never be removed). `check` says whether an action may happen for these inputs and names every condition's verdict in words; `execute` runs it through the mapped TM Forum capability **with the caller's own token** (a customer upgrades their own line, a clerk what their role allows), then writes a decision receipt into insight's decision log with the verdicts as evidence. The same definitions generate the **Ontology MCP** tools an agent sees (`upgrade_subscription`, `check_upgrade_subscription`, `explain`, `available_upgrades`) and the **TypeScript SDK** (`packages/genalpha-sdk`, regenerated and diffed by the suite), explain every console page to the ? drawer, and describe the component at `/.well-known/genalpha-component.json`. First journey: `upgradeSubscription` end to end — line, available upgrades, nine conditions, TMF622 modify order, OCS and entitlement effects, proration, events, receipt. Honest edge: one action so far; other components do not yet self-describe at runtime (suite `ontology_test.js`, `docs/ontology-seam.md`, research `docs/ontology-research.md`) |
