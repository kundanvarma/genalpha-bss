...
**Every feature is verified end-to-end in a real browser** — fifty-two Playwright suites drive the
...
suggestion becomes at most an "In study" DRAFT a product owner must promote), and
...
| admin-console | `/console` | Back office with **role-scoped tabs**: catalog, stock, campaigns, business Rules (with dry-run), porting, AI audit, the **Product Copilot** (chat → proposal card → "Yes — create it"), and a **Staff tab** (TMF672) where a tenant admin grants/revokes whole areas per operator — no IdP console needed. Each area appears only for operators holding its staff role |
