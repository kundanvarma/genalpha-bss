 ...
 * (macOS `say`, offline). It drives the REAL running stack across two live
 * sessions — the back-office console (as `demo`) and a customer's shop (as
 * `kai`) — so marketing is shown BOTH being authored and being received.
 *
...
function speak(voice, text) { if (!DRY) spawnSync('say', ['-v', voice, '-r', '186', text], { stdio: 'ignore' }); }
...
async function ensureOverlay(page) {
  await page.evaluate(() => {
    ...
    const badge = document.createElement('div'); badge.id = 'demo-badge'; badge.innerHTML = '<b></b> LIVE · AI-narrated demo';
...
async function narrate(page, persona, text) {
  ...
  await sleep(400); speak(p.voice, text); await sleep(250);
}
...
(async () => {
  ...
  const consoleCtx = await browser.newContext(vp); const cPage = await consoleCtx.newPage();
  const shopCtx = await browser.newContext(vp); const sPage = await shopCtx.newPage();
  ...
  await tryDo(async () => { await sPage.mouse.wheel(0, 500); }); await sleep(700);
  await narrate(sPage, 'ai',
  ...
  await clickTab(cPage, 'Pipeline board'); await sleep(500);
  ...
  await dropTitle(cPage);
  await narrate(cPage, 'mia', "I am Mia, in marketing. Let me build a campaign. New campaign — I pick a proven recipe, a real trigger like an order, and a message.");
  await clickTab(cPage, 'Campaigns');
  await tryDo(async () => { await cPage.getByRole('button', { name: /^\+?\s*new$/i }).first().click(); await sleep(1100); });
  await narrate(cPage, 'mia', "The AI drafts the copy — and the double-brace inserts the customer's name, the promo code drops in, and a holdout group is held back so I can prove the lift in money.");
  ...
  await tryDo(async () => { await cPage.locator('#list tbody tr, .row').first().click(); await sleep(1400); });
  ...
  await dropTitle(sPage);
  await tryDo(async () => { await sPage.locator('a:has-text("Notifications"), [href*="notification"]').first().click(); await sleep(1500); });
  await narrate(sPage, 'kai',
