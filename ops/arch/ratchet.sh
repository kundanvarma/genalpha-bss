...
for svc in sorted(os.listdir('services')):
    ...
    for dp, _, fs in os.walk(root):
        for f in fs:
            if f.endswith('.java'):
                with open(os.path.join(dp, f), errors='ignore') as fh: n += len(pat.findall(fh.read()))
...
if not quick: print(f"ratchet ok: untyped public returns {tm} (baseline {tb}); oversized front-end files {len(now['frontendFiles'])} (baseline {len(base['frontendFiles'])})")
PY
