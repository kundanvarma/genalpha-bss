...
(async () => {
  ...
  if (!lines.some((l) => l.startsWith('Chosen:')) || !lines.some((l) => l.includes(ref))) fail(`console receipt: ${JSON.stringify(lines)}`);
  ...
  if (!state.includes('v2')) fail(`contract row state: ${state}`);
  ...
  await page.fill('[data-testid=contract-allowed]', 'holdout, A, B');
  ...
  await page.fill('[data-testid=dryrun-candidates]', 'holdout, A, B');
  await page.fill('[data-testid=dryrun-holdout]', '50');
  await page.click('[data-testid=dryrun-run]');
  await page.locator('[data-testid=dryrun-result]', { hasText: 'Would choose' }).waitFor({ timeout: 10000 });
  const dryText = await page.locator('[data-testid=dryrun-result]').textContent();
  if (!dryText.includes('capped at 20 %') || !dryText.includes('@3')) fail(`console dry run: ${dryText}`);
  console.log(`  console Learning contracts: edited to v3 (unpaused, B allowed, cap 20 %); dry run says ${dryText.split('.')[0]}.`);
  await browser.close();
