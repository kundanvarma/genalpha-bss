...
(async () => {
  ...
  // the partner ring's ceiling is 25 per 2s (compose knob) — storm past it
  const storm = await Promise.all(Array.from({ length: 32 }, () =>
    ctx.get(`${API}/dealer/v1/kits`, { headers: H(pos) })));
  const refused = storm.filter((r) => r.status() === 429);
  if (!refused.length) fail('32 rapid POS knocks and no 429 — the rate limit is decorative');
  ...
  console.log('OK THE STORM IS CONTAINED: 32 rapid POS knocks → 429s with Retry-After for'
