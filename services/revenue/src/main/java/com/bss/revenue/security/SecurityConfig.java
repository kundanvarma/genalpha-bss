package com.bss.revenue.security;
 ...
 * other issuer are rejected before authorization runs. Agreements are
 * personal consumption data. Reads require "agreement:read" (customers
 * party-scoped); writes "agreement:write" — the ingest endpoint is the
 * mediation/OCS seam, machine and back-office only. Health and the OpenAPI
    ...
    SecurityFilterChain apiSecurity(HttpSecurity http, ClaimAuthoritiesConverter authoritiesConverter,
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // finance-grade, staff-only: journal reads ride
                        // billing:read; remapping the chart and backfill are
                        // billing:admin. No customer-facing surface exists.
                        .requestMatchers(HttpMethod.POST, ApiConstants.BASE_PATH + "/accountMapping").hasAuthority("billing:admin")
                        .requestMatchers(HttpMethod.POST, ApiConstants.BASE_PATH + "/backfill").hasAuthority("billing:admin")
                        .requestMatchers(HttpMethod.GET, ApiConstants.BASE_PATH + "/**").hasAuthority("billing:read")
                        .requestMatchers(ApiConstants.BASE_PATH + "/**").hasAuthority("billing:admin")
