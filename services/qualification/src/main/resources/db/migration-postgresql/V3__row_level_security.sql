...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'qualification_app') THEN
        CREATE ROLE qualification_app LOGIN PASSWORD 'qualification_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO qualification_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO qualification_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO qualification_app;

ALTER TABLE serviceable_area ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON serviceable_area
