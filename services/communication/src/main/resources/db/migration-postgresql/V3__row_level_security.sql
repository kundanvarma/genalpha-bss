...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'communication_app') THEN
        CREATE ROLE communication_app LOGIN PASSWORD 'communication_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO communication_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO communication_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO communication_app;

ALTER TABLE communication_message ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON communication_message
