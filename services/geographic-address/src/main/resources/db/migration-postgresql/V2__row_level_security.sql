...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'address_app') THEN
        CREATE ROLE address_app LOGIN PASSWORD 'address_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO address_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO address_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO address_app;

ALTER TABLE geographic_address ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON geographic_address
