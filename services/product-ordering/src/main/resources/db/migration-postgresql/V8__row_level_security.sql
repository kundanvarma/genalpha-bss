...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'ordering_app') THEN
        CREATE ROLE ordering_app LOGIN PASSWORD 'ordering_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO ordering_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ordering_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ordering_app;

ALTER TABLE product_order ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON product_order
