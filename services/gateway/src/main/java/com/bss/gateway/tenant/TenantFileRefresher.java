 ...
 * service's Entry knows and ignores the rest.
 */
@Component
public class TenantFileRefresher {
    ...
    public TenantFileRefresher(TenantHosts tenants) {
    ...
    public void refresh() {
        try (FileInputStream in = new FileInputStream(file)) {
            ...
            for (Map<String, Object> entry : (List<Map<String, Object>>) block.get("registry")) {
                // tenants.yml is operator-editable and a hosted operator's own
                // team can reach it. An entry with no id used to become a
                // tenant literally called "null", which then serves requests.
                // Refuse the entry, name it, keep the rest of the fleet.
                ...
                tenants.getRegistry().add((TenantHosts.Entry) fresh);
    ...
    private Object newEntry() throws Exception {
        for (Class<?> inner : TenantHosts.class.getDeclaredClasses()) {
        ...
        throw new IllegalStateException("no Entry inner class");
    }
