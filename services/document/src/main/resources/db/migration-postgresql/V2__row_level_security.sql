...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'document_app') THEN
        CREATE ROLE document_app LOGIN PASSWORD 'document_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO document_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO document_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO document_app;
