...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'appointment_app') THEN
        CREATE ROLE appointment_app LOGIN PASSWORD 'appointment_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO appointment_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO appointment_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO appointment_app;
