...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'assurance_app') THEN
        CREATE ROLE assurance_app LOGIN PASSWORD 'assurance_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO assurance_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO assurance_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO assurance_app;

ALTER TABLE alarm ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON alarm
    USING (current_setting('app.tenant_id', true) = '__system__'
           OR tenant_id = current_setting('app.tenant_id', true))
    WITH CHECK (current_setting('app.tenant_id', true) = '__system__'
           OR tenant_id = current_setting('app.tenant_id', true));
ALTER TABLE service_problem ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON service_problem
    USING (current_setting('app.tenant_id', true) = '__system__'
           OR tenant_id = current_setting('app.tenant_id', true))
    WITH CHECK (current_setting('app.tenant_id', true) = '__system__'
           OR tenant_id = current_setting('app.tenant_id', true));
