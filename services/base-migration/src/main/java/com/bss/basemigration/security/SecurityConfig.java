package com.bss.basemigration.security;
 ...
 * other issuer are rejected before authorization runs. Base migration is a
 * pure back-office desk that moves the installed base: reads require
 * "migration:read", every mutation "migration:admin" — customers never
 * talk to this API, they meet its output as notices and orders. Health
 * and the OpenAPI docs stay open for probes and discovery.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {

    private static final String READ = "migration:read";
    private static final String ADMIN = "migration:admin";
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, ApiConstants.BASE_PATH + "/**")
                                .hasAnyAuthority(READ, ADMIN)
                        .requestMatchers(HttpMethod.POST, ApiConstants.BASE_PATH + "/**").hasAuthority(ADMIN)
                        .requestMatchers(HttpMethod.PATCH, ApiConstants.BASE_PATH + "/**").hasAuthority(ADMIN)
                        .requestMatchers(HttpMethod.DELETE, ApiConstants.BASE_PATH + "/**").hasAuthority(ADMIN)
