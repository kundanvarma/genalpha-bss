...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'som_app') THEN
        CREATE ROLE som_app LOGIN PASSWORD 'som_app';
    END IF;
END
$$;
GRANT USAGE ON SCHEMA public TO som_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO som_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO som_app;

ALTER TABLE service_order ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON service_order
