...
// A brand color is picked for identity, not contrast — as small text on the
// soft tint it can fall below WCAG AA 4.5:1. Derive a readable shade (darker on
// light backgrounds, lighter on dark) so any tenant brand stays legible.
function readableText(hex, bgHex) {
  ...
  const bg = rgb(bgHex);
  const darken = lum(bg) > 0.5;
  let c = rgb(hex);
  for (let i = 0; i < 48 && ratio(c, bg) < 5.5; i++) {
