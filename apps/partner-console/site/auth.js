...
const TOKEN_KEY = 'bss.partner.token';
const EXP_KEY = 'bss.partner.tokenExp';
const VERIFIER_KEY = 'bss.partner.verifier';
const STATE_KEY = 'bss.partner.state';
...
async function authFetch(url, options) {
  ...
  options = { ...(options || {}), headers: { 'X-Channel': 'partner', ...((options && options.headers) || {}) } };
  const token = currentToken();
  if (!token) {
    await beginLogin();
